...
async def process_withdrawal(
    ...
    if data.status == "completed" and withdrawal.method.lower() == "wompi":
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp = await client.post(
                f"{WOMPI_API_BASE}/payouts", json=payload, headers=headers
            )
            if resp.status_code != 201:
                raise HTTPException(
                    status_code=400, detail=f"Error en Dispersión Wompi: {resp.text}"
                )
    ...
    if data.status == "rejected":
        ...
        owner_result = await db.execute(
            select(DBUser).where(DBUser.id == withdrawal.owner_id)
        )
...
async def get_owner_summary(
    ...
    rev_result = await db.execute(
        select(func.sum(Payment.owner_amount))
...
@app.get(
    "/owner/channels/{channel_id}/promotions", response_model=List[PromotionResponse]
)
...
async def bot_check_promo(
    code: str, user_id: int, db: AsyncSessionLocal = Depends(get_db)
):
    """
...
async def create_payment_link(
    ...
    if data.method == "stripe":
        ...
        wallet_address = os.getenv(
            "CRYPTO_WALLET_ADDRESS", "TXYZ... (Configura tu wallet)"
        )
