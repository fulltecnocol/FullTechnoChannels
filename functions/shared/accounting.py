...
from .models import User, Payment, Channel, Plan, SystemConfig, AffiliateEarning
        ...
        if level == 1:
            ...
            if tier == "DIAMANTE":
                level_fee_percent = await get_config_value(
                    db, "affiliate_level_1_fee_diamond", 0.08
                )
            elif tier == "ORO":
                level_fee_percent = await get_config_value(
                    db, "affiliate_level_1_fee_gold", 0.05
                )
            else:
                level_fee_percent = await get_config_value(
                    db, "affiliate_level_1_fee", 0.03
                )
        else:
