    ...
    async with AsyncSessionLocal() as session:
        ...
        if args and args.startswith("ref_"):
            ...
            if referrer and not user.referred_by_id and user.id != referrer.id:
                ...
                await session.commit()
                await message.reply(
                    f"🎯 **¡Te has unido a la red de {referrer.full_name}!**"
                )
        ...
        if args:
            ...
            promo_res = await session.execute(
                select(Promotion).where(
                    and_(Promotion.code == args, Promotion.is_active)
                )
            )
            ...
            result = await session.execute(
                select(Channel).where(Channel.validation_code == args)
            )
        ...
        await message.reply(
            "¡Hola! Soy tu bot de membresía multi-canal. Usa un link de invitación para empezar."
        )
