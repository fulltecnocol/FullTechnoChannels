...
async def register_user(client, email, password, full_name, request_id):
    ...
    try:
        ...
        return {"id": request_id, "status": "error", "error": str(e), "duration": 0}
...
async def run_test():
    ...
    print(f"👤 User: {email}")
    print("🚀 Launching 3 concurrent registration requests...")
    ...
    for res in sorted_results:
        print(
            f"   Request #{res['id']}: Status {res['status']} ({res['duration']:.2f}s)"
        )
        if res["status"] == 200:
            success_count += 1
    ...
    if success_count == 1 and fail_400_count >= 1:
        ...
        print(
            "❌ FAILED: Multiple registrations succeeded (Double Registration still exists!)."
        )
    elif success_count == 0:
        print("❌ FAILED: All requests failed.")
    else:
        print(
            f"⚠️ INDETERMINATE: Success={success_count}, 400s={fail_400_count}, Others={fail_other_count}"
        )
