...
export interface SupportSectionProps {
    ...
    onCreateTicket?: (subject: string, message: string, priority: string) => Promise<void>;
    user?: any;
}

export function SupportSection({ tickets, isAdmin = false, onAdminViewTicket, onCreateTicket, user }: SupportSectionProps) {
    const handleCreateClick = () => {
        if (onCreateTicket) {
            const subject = prompt("Asunto del Ticket:");
            if (subject) {
                const message = prompt("Mensaje:");
                if (message) onCreateTicket(subject, message, "normal");
            }
        }
    };
                    ...
                    {tickets.length > 0 ? tickets.map((ticket) => (
                        ...
                        <div className="p-8 text-center space-y-4">
                            ...
                            {!isAdmin && (
                                <button
                                    onClick={handleCreateClick}
                                    className="w-full py-3 bg-primary text-primary-foreground rounded-xl font-bold shadow-lg shadow-primary/20 text-xs"
                                >
                                    Crear Ticket
                                </button>
                            )}
