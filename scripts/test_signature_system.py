...
def test_pdf_generation():
    ...
    try:
        ...
        output_path = (
            Path(__file__).parent.parent / "test_output" / "preview_contract.pdf"
        )
        ...
        print("   Puedes abrirlo para verificar el diseño")
...
def test_blockchain_service():
    ...
    try:
        ...
        print("\n🔍 Configuración:")
        print(
            f"   RPC URL configurado: {'✅' if os.getenv('POLYGON_RPC_URL') else '❌'}"
        )
        print(
            f"   Contract Address configurado: {'✅' if os.getenv('CONTRACT_REGISTRY_ADDRESS') else '❌'}"
        )
        print(
            f"   Signer Address configurado: {'✅' if os.getenv('SIGNER_ADDRESS') else '❌'}"
        )
        print(
            f"   Private Key configurado: {'✅' if os.getenv('SIGNER_PRIVATE_KEY') else '❌'}"
        )
        ...
        if is_connected:
            print("✅ Conexión exitosa a Polygon")
            ...
            if os.getenv("CONTRACT_REGISTRY_ADDRESS"):
                ...
                print("   2. npm install")
                print("   3. npm run deploy:mumbai")
                print("   4. Agregar dirección al .env")
        else:
            print("⚠️  No conectado (esperado si RPC_URL no está configurado)")
            print("\n   Para habilitar:")
            print("   Agregar al .env:")
            print("   POLYGON_RPC_URL=https://polygon-rpc.com")
