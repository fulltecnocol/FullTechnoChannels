...
def link_telegram(email, telegram_id):
    ...
    with engine.connect() as conn:
        # Check user
        result = conn.execute(text("SELECT id, email, telegram_id FROM users WHERE email=:email"), {"email": email}).fetchone()
        if not result:
        ...
        # Update details
        conn.execute(
            text("UPDATE users SET telegram_id=:tg_id, is_owner=true WHERE email=:email"),
