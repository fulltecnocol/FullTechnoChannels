...
async def test_environment(suite: TestSuite):
    ...
    for var in optional_vars:
        value = os.getenv(var)
        if not value:
            suite.warn(f"Optional variable {var} not set", "Payment features may not work")
        ...
        try:
            async with engine.begin() as conn:
                ...
                for table in expected_tables:
                    ...
                    if not exists:
                        suite.warn(f"Table '{table}' exists in database", f"Table not found - migrations may be needed")
                    else:
...
async def test_dependencies(suite: TestSuite):
    ...
    for dep in critical_deps:
        try:
            ...
            suite.test(f"Dependency '{dep}' installed", False, f"Run: pip install {dep}")
