...
async def test_bot_logic():
    ...
    tg_id = 123456789
    mock_tg_user = MockUser(
        id=tg_id, username="test_bot_user", full_name="Test Bot User"
    )
    ...
    async with AsyncSessionLocal() as session:
        ...
        print(
            f"OK: User created/retrieved. ID: {user.id}, Telegram ID: {user.telegram_id}"
        )
    ...
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(select(DBUser).where(DBUser.is_owner))
        ...
        if not web_user:
            ...
            print(
                f"   Target Web User: {web_user.email} (Ref Code: {web_user.referral_code})"
            )

            # Simulate /start sync_REFCODE from a NEW telegram user
            new_tg_id = 987654321
            new_tg_user = MockUser(
                id=new_tg_id, username="linker_user", full_name="Linker User"
            )
            ...
            try:
                ...
                if web_user.telegram_id == new_tg_id:
                    print(
                        f"SUCCESS: Web user {web_user.email} linked to Telegram ID {new_tg_id}"
                    )
                else:
                    print(
                        f"FAILED: Link not updated. Current Telegram ID: {web_user.telegram_id}"
                    )
