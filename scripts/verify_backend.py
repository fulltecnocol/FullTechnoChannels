...
def test_backend():
    ...
    try:
        reg_response = httpx.post(f"{BASE_URL}/register", json={
            "email": email,
            "password": password,
            "full_name": full_name
        })
        ...
        print(f"OK: User Registered. Token received: {token_data.get('access_token')[:10]}...")
    ...
    try:
        login_response = httpx.post(f"{BASE_URL}/token", data={
    ...
    try:
        summary_response = httpx.get(f"{BASE_URL}/owner/dashboard/summary", headers=headers)
        if summary_response.status_code != 200:
            print(f"FAILED: Get Summary {summary_response.status_code} - {summary_response.text}")
            sys.exit(1)
        
        summary_data = summary_response.json()
        print(f"OK: Summary Verified. Active Subs: {summary_data['active_subscribers']}, Revenue: {summary_data['available_balance']}")
    except Exception as e:
    ...
    try:
        chan_response = httpx.post(f"{BASE_URL}/owner/channels", headers=headers, json={
            "title": channel_title
        })
        
        if chan_response.status_code in [200, 201]:
            chan_data = chan_response.json()
            print(f"OK: Channel Created (ID: {chan_data['id']}, Code: {chan_data['validation_code']})")
            channel_id = chan_data['id']
        else:
            print(f"FAILED: Channel Creation {chan_response.status_code} - {chan_response.text}")
            sys.exit(1)
