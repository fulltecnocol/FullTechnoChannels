...
def test_template_exists():
    ...
    try:
        ...
        if template_path.exists():
            ...
            if all_ok:
                print(f"✅ Todas las variables clave presentes")
...
def test_blockchain_import():
    ...
    try:
        ...
        print(f"\n🔍 Configuración:")
        print(f"   RPC URL: {'✅ configurado' if os.getenv('POLYGON_RPC_URL') else '❌ no configurado'}")
        print(f"   Contract Address: {'✅ configurado' if os.getenv('CONTRACT_REGISTRY_ADDRESS') else '❌ no configurado (normal en desarrollo)'}")
...
def test_requirements():
    ...
    try:
        ...
        print(f"✅ WeasyPrint (PDF generation)")
    except Exception as e:
        print(f"⚠️  WeasyPrint - {str(e)[:50]}...")
        print(f"   (Requiere librerías del sistema en macOS)")
        print(f"   Puedes usar alternativas como wkhtml topdf o usar en producción (Linux)")
