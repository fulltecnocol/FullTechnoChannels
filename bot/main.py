    ...
    while True:
        try:
            async with AsyncSessionLocal() as session:
                ...
                # Buscar suscripciones que vencen mañana y no han sido notificadas (podríamos añadir un flag 'notified')
                # Por ahora, simplemente las que vencen en las próximas 24h
                result = await session.execute(
                    select(Subscription, DBUser, Channel).join(DBUser, Subscription.user_id == DBUser.id).join(Plan, Subscription.plan_id == Plan.id).join(Channel, Plan.channel_id == Channel.id).where(
                        and_(
                ...
                for sub, user, channel in result.all():
                    if user.telegram_id:
                        msg = channel.expiration_message or f"⚠️ **Aviso VIP**: Tu suscripción a '{channel.title}' vence en menos de 24 horas. ¡Renuévala ahora para mantener tu acceso!"
                        try:
                            await bot.send_message(user.telegram_id, msg, parse_mode="Markdown")
                        except:
                            pass
        except Exception as e:
            logging.error(f"Error in expiration loop: {e}")
...
async def on_startup():
    ...
    asyncio.create_task(check_expirations_loop())
