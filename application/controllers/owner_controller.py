...
from application.dto.user import DashboardSummary, UserProfileResponse, ProfileUpdate
from application.dto.auth import PasswordUpdate
from application.dto.channel import ChannelCreate, BrandingUpdate
from application.dto.plan import PlanResponse, PlanCreate, PlanUpdate
from application.dto.misc import (
    PromotionResponse,
    PromotionCreate,
    WithdrawalRequest,
    TicketCreate,
...
from application.middlewares.auth import get_current_owner, oauth2_scheme
...
async def get_owner_summary(
    ...
    print(f"DEBUG: get_owner_summary for user_id={current_user.id}, email={current_user.email}")
    # 1. Canales Activos
...
async def update_password(
    ...
    if not is_recovery:
        if not data.current_password:
            raise HTTPException(
                status_code=400, detail="Se requiere la contraseña actual"
            )

        if current_user.hashed_password and not AuthService.verify_password(
...
async def check_channel_delete_cost(
    channel_id: int,
    current_user: DBUser = Depends(get_current_owner),
    db: AsyncSessionLocal = Depends(get_db),
):
...
async def delete_channel(
    ...
    if active_subs:
        ...
        for sub in active_subs:
            ...
            if remaining_days > 0 and sub.plan.duration_days > 0:
                user_compensation = (
                    sub.plan.price / sub.plan.duration_days * remaining_days
                ) * 1.20
                if sub.user:
...
async def get_channel_plans(
    channel_id: int,
    current_user: DBUser = Depends(get_current_owner),
    db: AsyncSessionLocal = Depends(get_db),
):
    chan_check = await db.execute(
        select(Channel).where(
            and_(Channel.id == channel_id, Channel.owner_id == current_user.id)
        )
    )
...
async def update_plan(
    ...
    result = await db.execute(
        select(Plan)
        .join(Channel)
        .where(and_(Plan.id == plan_id, Channel.owner_id == current_user.id))
    )
...
async def delete_plan(
    ...
    subs = await db.execute(
        select(Subscription).where(
            and_(Subscription.plan_id == plan_id, Subscription.is_active)
        )
    )
...
async def get_channel_promotions(
    channel_id: int,
    current_user: DBUser = Depends(get_current_owner),
    db: AsyncSessionLocal = Depends(get_db),
):
    ...
    result = await db.execute(
        select(Promotion).where(Promotion.channel_id == channel_id)
    )
    return result.scalars().all()
...
async def delete_promotion(
    ...
    result = await db.execute(
        select(Promotion)
        .join(Channel)
        .where(and_(Promotion.id == promo_id, Channel.owner_id == current_user.id))
    )
...
async def request_withdrawal(
    ...
    new_withdrawal = Withdrawal(
        owner_id=current_user.id, **data.dict(), status="pending"
    )
...
async def get_my_withdrawals(
    ...
    result = await db.execute(
        select(Withdrawal)
        .where(Withdrawal.owner_id == current_user.id)
...
async def create_ticket(
    ...
    new_ticket = SupportTicket(
        user_id=current_user.id, subject=data.subject, priority=data.priority
    )
    db.add(new_ticket)
    await db.flush()
    initial_msg = TicketMessage(
        ticket_id=new_ticket.id, sender_id=current_user.id, content=data.content
    )
    db.add(initial_msg)
    await db.commit()
    return new_ticket
...
async def reply_ticket_owner(
    ...
    new_msg = TicketMessage(
        ticket_id=ticket_id, sender_id=current_user.id, content=data.content
    )
    ticket.status = "open"
    db.add(new_msg)
    await db.commit()
...
async def get_owner_analytics(
    ...
    """Ingresos y crecimiento de suscriptores (últimos 30 días)"""
    from datetime import timedelta
    thirty_days_ago = datetime.utcnow() - timedelta(days=30)

    # 1. Ingresos diarios (Dueño)
    ...
    subscriber_chart = [
        {"date": str(s.date), "value": s.count} for s in sub_query.all()
    ]
    ...
    mlm_query = await db.execute(
        ...
        .group_by(func.date(AffiliateEarning.created_at))
        .order_by(func.date(AffiliateEarning.created_at))
    )
