...
from core.entities import OwnerLegalInfo, SignatureCode, SignedContract
...
from application.middlewares.auth import get_current_user
...
async def verify_signature(
    ...
    # 4. Guardar PDF usando StorageFactory
    storage = StorageFactory.get_provider()
    filename = f"contract_{current_user.id}_{int(datetime.utcnow().timestamp())}.pdf"
    await storage.save_file(pdf_bytes, filename)
    
    pdf_url = filename # Guardamos el nombre del archivo o identificador
...
async def download_contract(
    ...
    # En local devolvemos la URL configurada por el provider si es para descarga directa
    # o el contenido si fuera necesario. Aquí devolvemos el status y la URL para el front.
    public_url = storage.get_public_url(pdf_url)
