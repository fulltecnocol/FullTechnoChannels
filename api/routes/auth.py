...
from api.schemas.auth import Token, UserRegister, GoogleAuthRequest, GenerateTokenRequest, CreateMagicLinkRequest
            ...
            if user:
                ...
                if auth_data.registration_token:
                    ...
                    if token_obj:
                        ...
                        tg_check = await db.execute(select(DBUser).where(DBUser.telegram_id == telegram_id))
                        if tg_check.scalar_one_or_none():
                             raise HTTPException(status_code=400, detail="Esta cuenta de Telegram ya está vinculada a otro usuario.")
...
async def magic_login(token: str, db: AsyncSessionLocal = Depends(get_db)):
    ...
    if not payload or payload.get("type") != "magic_link":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Enlace inválido o expirado")
    ...
    access_token = AuthService.create_access_token(data={"sub": user.email, "recovery": True})
    return {"access_token": access_token, "token_type": "bearer"}
